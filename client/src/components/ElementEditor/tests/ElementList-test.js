...
test('ElementList still resolves entwineResolve when the elements prop updates mid batch save', () => {
  ...
  const makeListProps = (listElements) => makeProps({
  ...
  const { rerender } = render(<ElementList {...makeListProps(elements)} />);
  ...
  act(() => {
    rerender(<ElementList {...makeListProps(elements.map(e => ({ ...e })))} />);
  });
