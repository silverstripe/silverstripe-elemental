...
Feature: Blocks are validated when page saving blocks
  ...
  Background:
    Given I add an extension "SilverStripe\FrameworkTest\Elemental\Extension\MultiElementalAreasExtension" to the "Page" class
  ...
  Scenario: Validation when page saving inline blocks
    ...
    # order for it be submitted on page save
    ...
    And I click "Choose existing" in the "#Form_EditForm_ElementalArea1 .uploadfield" element
    ...
    And I click on the ".ss-searchable-dropdown-field__option:nth-of-type(2)" element
    And I click "Choose existing" in the "#Form_EditForm_ElementalArea2 .uploadfield" element
    And I press the "Back" HTML field button
    And I click on the file named "file1" in the gallery
    And I press the "Insert" button
    And I click on the caret button for block 2
    ...
    # We should see a failure toast for the invalid page
